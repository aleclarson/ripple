...
internal const val RIPPLE_PLUGIN_ID = "com.ripple_ts.intellij_plugin"

internal fun copyDirectory(source: Path, target: Path): Boolean {
	return runCatching {
...
internal fun copyFromJar(resourceUrl: URL, target: Path): Boolean {
	return runCatching {
...
internal fun pluginVersion(): String {
	val descriptor = PluginManagerCore.getPlugin(PluginId.getId(RIPPLE_PLUGIN_ID))
	return descriptor?.version ?: "dev"
}

internal fun normalizeConfiguredValue(value: String?): String? {
...
internal fun resolveConfiguredPath(value: String?): Path? {
	val normalized = normalizeConfiguredValue(value) ?: return null
	return runCatching {
